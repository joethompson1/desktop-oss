...
> The agent sidecars and their native binaries are built for your machine automatically — `npm install` wires them up and `npm run tauri:build` bundles the right architecture. Per-OS/arch release installers are built in CI (push a `v*` tag). See [CLAUDE.md](./CLAUDE.md#sidecars-architecture--releases) for details.
