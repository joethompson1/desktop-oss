...
- **Bring your own LLM.** Configure one or more harnesses: Anthropic (API
...
On first run the app opens **Settings → Harnesses**. Add at least one
harness (e.g. paste an Anthropic API key, or point at a local Ollama
...
The orchestrator (the chat you talk to) must be a raw-LLM harness
(Anthropic or OpenAI-compatible). The agentic harnesses (Claude Code,
...
```
src/lib/harnesses  bring-your-own-LLM backends
