...
Some delegates are **terminal-capable** (currently the Claude Code harness): passing \`surface: "tui"\` to delegate_task opens the agent's real CLI in an interactive terminal that the user drives directly. Only do this when the user explicitly asks for a terminal / interactive session — a tui delegate does nothing until the user engages with it, and message_delegate is unavailable while it's in terminal mode (get_delegate_history still works; its conversation is mirrored live).
