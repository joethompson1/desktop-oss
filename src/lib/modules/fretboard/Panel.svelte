        ...
        {#each visibleSteps as vs, i (vs.index)}
          <span class="crumb-slot" animate:flip={{ duration: 220, easing: cubicOut }} in:fade={{ duration: 150 }} out:fade={{ duration: 100 }}>
            <button
              ...
              aria-current={vs.index === state.stepIndex ? "step" : undefined}
    ...
    >
      ...
      {#each strings as s (s)}
        ...
        {#if open}
          ...
          >
            <!-- Root markers get a ring (non-colour cue) so root vs non-root
                 survives greyscale / colour-blind viewing, not just accent fill.
                 The label is #fff on the accent-filled root circle: an
                 intentional on-accent foreground — there's no on-accent theme
                 token (--accent-text is a link colour, not a contrast fg). -->
            <circle
              cx={stringX(s, dims)}
              cy={fretCenterY(0, dims)}
              r="9"
              fill={open.root ? "var(--accent)" : "var(--text)"}
              stroke={open.root ? "var(--text)" : "none"}
              stroke-width={open.root ? "2" : "0"}
            />
            {#if open.label}
              <text
                x={stringX(s, dims)}
      ...
      {#each fretted as m (`${m.string}:${m.fret}`)}
        ...
        >
          <circle
            ...
            stroke={m.root ? "var(--text)" : "none"}
            stroke-width={m.root ? "2" : "0"}
          />
          {#if m.label}
            <text
