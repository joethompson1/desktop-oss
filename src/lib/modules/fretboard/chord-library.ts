...
export interface ChordShape {
  ...
  root: string;
  /** The exact set of pitch classes this shape SOUNDS (canonical sharp
   *  spelling) — used only by the self-check test, not read at runtime. The
   *  test asserts this equals the shape's sounded notes exactly (both
   *  directions), so it must not list a chord tone the voicing omits. */
  tones: string[];
}
...
export const CHORD_LIBRARY: Record<string, ChordShape> = {
  ...
  // Common open C7 voicing (x32310) — the 5th (G) is omitted, so it is not
  // listed in `tones` (which is the sounded set, not the theoretical spelling).
  C7: { frets: [0, 1, 3, 2, 3, null], root: "C", tones: ["C", "E", "A#"] },
