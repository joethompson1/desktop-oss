...
export default defineModule<FretboardState>({
  ...
  icon: "♪",
  description:
    "Draw chords, scales, and chord progressions on a guitar neck the agent can control.",
  version: "1.0.0",
  author: "desktop-oss",
  // A single-purpose module in a generic shell ships OFF — the user opts in
  // from Settings → Modules. Keeps a coding/research session from being handed
  // guitar-tutor tools + a prompt fragment it never asked for.
  ...
  // Kept short on purpose: the per-argument detail (chord/movableShape/notes,
  // tab numbering, etc.) lives in the tool + schema descriptions, which the
  // model already receives — no need to restate it here every turn. This is
  // just the panel's existence plus the two non-obvious nudges.
  promptFragment: () =>
    "## Fretboard panel\n" +
    ...
    "neck — prefer its `chord`/`movableShape` inputs over guessing frets via " +
    "`notes` (note names and root highlighting are computed for you). For " +
    "several chords in a row (a song's changes, a I-IV-V, a drill), call " +
    "`fretboard_show_progression` instead of repeating `fretboard_show`.",
});
